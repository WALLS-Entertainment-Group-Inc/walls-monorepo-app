...
/**
 * CRM is an account-scoped app: every project belongs to a WALLS account
 * (personal or organization). This module resolves the current user, the
 * accounts they belong to, and the "active" account (whose CRM data is shown),
 * which is persisted in a cookie and validated against membership on every read.
 */

export const CRM_ACCOUNT_COOKIE = "crm_account_id";
...
function mapMembership(row: AccountMembershipRow): CrmAccount | null {
...
export async function listAccountsForUser(
  ...
  const accounts = (data ?? [])
    ...
    .filter((account): account is CrmAccount => account !== null);
